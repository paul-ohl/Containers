...
/*   Updated: 2022/01/25 17:16:43 by pohl             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
public:
	...
	map_iterator&	operator++( void )
	{
		...
		while (tmp->getParent())
			tmp = tmp->getParent();
		tmp = tmp->upper_bound(this->_node->getKey());
		if (!tmp)
			this->_is_end = true;
		else
			this->_node = tmp;
	...
	map_iterator&	operator--( void )
	{
		...
		this->_node = this->_node->under_bound(key);
		return *this;
	}  
...
public:
	...
	// Member functions
	const_map_iterator( node *ptr = NULL, bool is_end = false ) :
		_node(ptr), _is_end(is_end)
	{
		return;
	}
	const_map_iterator( const const_map_iterator<const Key, T, Compare> &other ):
		_node(other.getNode()), _is_end(other._is_end)
	{
		return;
	}
	const_map_iterator( map_iterator<const Key, T, Compare> other ):
	...
	const_map_iterator &operator=( const const_map_iterator<const Key, T, Compare> &other )
	{
		this->_node = other._node;
		this->_is_end = other._is_end;
		return *this;
